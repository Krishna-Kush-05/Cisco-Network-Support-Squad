...
# ---------------------------------------------------------------------------
# __main__ — quick manual test against a sample case_XX.txt file
# ---------------------------------------------------------------------------
...
def _load_case_file(path: str) -> Case:
    with open(path, "r", encoding="utf-8") as f:
        raw = f.read()

    m = _CASE_FILE_RE.search(raw)
    if not m:
        raise ValueError(
            f"Could not parse {path} — expected CASE_ID/TOPOLOGY/FAULT_TYPE/"
            f"SYMPTOM/GROUND_TRUTH fields followed by an "
            f"'--- EVIDENCE START ---' / '--- EVIDENCE END ---' block."
        )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python checker.py <case_XX.txt> [more_case_files...]")
        sys.exit(1)

    for path in sys.argv[1:]:
        case = _load_case_file(path)
        result = run_rule_check(case)
        print(f"\n=== {path} ===")
        ...
        for k in _FLAG_LABELS:
            print(f"  {k:18s}: {getattr(result, k)}")
